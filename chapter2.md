...
ソケットは常にvoidポインタであり、メッセージは構造体であることに注意してください。
...
ØMQのコネクションは、古典的なTCPコネクションとは幾つか異なる点があります。
主要な違いは以下の通りです。
...
[TODO]UDPとは異なり、殆どの通信方式では同じエンドポイントを2度バインドする事は出来ません。
IPC通信方式ではこれを行うことができますが、最初にbindを行ったプロセスのソケットは無効になります。これはプロセスのクラッシュから回復する為のものです。

Although ØMQ tries to be neutral about which side binds and which side connects, there are differences. We'll see these in more detail later.
...
The chances that it will "just work" are much better like that.

[TODO]とはいえ、ØMQはbindする側と接続する側について中立であろうとする事に大きな特徴があります。
後でもっと詳しく説明しますが結論だけ言うと、
...
恐らくは、普通のTCPソケットでHTTPリクエストとレスポンスを転送できるのだから、ØMQソケットを利用して同じ事をより良く実現できるのではないか、と考えているのでしょう。
...
## メッセージングパターン
Underneath the brown paper wrapping of ØMQ's socket API lies the world of messaging patterns.
If you have a background in enterprise messaging, or know UDP well, these will be vaguely familiar. But to most ØMQ newcomers, they are a surprise. We're so used to the TCP paradigm where a socket maps one-to-one to another node.
...
通信経路上で、ØMQのメッセージは0から任意のサイズのデータとしてメモリに格納されています。
そしてprotocol buffersやmsgpack、JSON、あるいはあなたのアプリケーションで利用できるものを利用して独自にシリアライゼーションを行います。
可搬性のあるデータ表現を利用する事は賢明ですが、この決定にはトレードオフが伴うでしょう。
...
ØMQがTCP接続上でフレームを読み書きする方法を定義したZMTPという転送プロトコルがあります。
...
繰り返し言いますが、まだ`zmq_msg_init_data()`関数を利用しないで下さい。
これはゼロコピーを行う手段であり、間違いなくあなたを悩ませます。
...
これは中身を読まずにメッセージを転送するようなプロキシーの様なアプリケーションで必要です。
...
ØMQは分散インテリジェンスを目指しますが、ネットワークの中間に何もないというわけではありません。
メッセージを扱うインフラやØMQで構築したインフラで満たされています。
ØMQは小さなパイプから、完全なサービス指向ブローカーまで様々な配管を行うことが可能です。
メッセージング業界では、中間でメッセージを取り扱う役割を仲介者と呼びます。
ØMQでは、この役割の事を文脈に依存してプロキシー、キュー、フォワーダー、デバイス、ブローカと呼びます。
...
ØMQではメッセージブローカーは居ませんが、とても簡単に仲介者を構築することが出来ます。
...
初心者の場合、それは適当な妥協策です。
常にスター型トポロジを利用すればパフォーマンスについては問題なく動作するでしょう。
...
この場合、仲介者をステートレスなメッセージスイッチとして考えた方が良いでしょう。
...
この設計でクライアントとサービスを追加するのは比較的容易にでしょう。
...
幸いにも、リクエスト・応答を非ブロッキングで行う2つのソケットはDEALERとROUTERと呼ばれています。
...
簡単な答えは、「通常はROUTER/DEALER, XSUB/XPUB, PULL/PUSH の組み合わせしか利用しません。」
...
致命的なエラーとして扱ってはいけない、例外的な状況が主に2つあります。
 ...
 * `ZMQ_DONTWAIT`を指定してメッセージを受信しようとして実際に受信するデータが無かった場合、ØMQは-1を返し、errnoにEAGAINをセットします。
...
C/C++の`assert()`は最適化によって取り除く事ができるので全てのØMQ内で呼び出される`assert()`をラップする必要はありません。
...
[TODO]ほとんどの並行プログラミングに関する本では普通の事です。
...
バグのあるプログラムの経験で世界一の大企業が「マルチスレッドプログラムでよく見られる11の問題」という文書を公開しました。
...
広く使われているモデルは業界の基盤となっているにも関わらず、状態を共有するという根本的に欠陥があります。
...
* [TODO]あなたのアプリケーションを構成するスレッドを生成したら、プロセス内通信でペアのソケットを利用して親スレッドに接続して下さい。親スレッド側のソケットでbindして、子スレッド側のソケットで接続を行うのが定石です。
...
例えばアプリケーションの中で2つ以上のプロクシを動作させたい場合、マルチスレッドを利用したい場合もあるでしょう。
1つのスレッド内でエラーが発生した際に、ソケットを別のスレッドのプロキシに渡すことも簡単です。

ソケットの生成を行ったスレッドでのみcloseを行なって下さい。

使用すると、アプリケーション内の複数のプロキシを起動する必要がある場合、たとえば、あなたは、自分のスレッドでそれぞれを実行することになるでしょう。これは、1つのスレッドでのプロキシフロントエンドとバックエンドのソケットを作成し、別のスレッドでプロキシにソケットを渡すエラーを作ることは容易である。これは、最初に動作するように表示されることがありますが、実際の使用中にランダムに失敗します。覚えている：それらを作成したスレッドを除き、または近いソケットを使用しないでください。
...
また、大量のスレッドを生成するとOSはストレスが受けてしまうでしょう。
...
もうコードを見たら何をやっているか理解できる頃でしょう。一応説明しておくと、
...
* 送信側でPUSH、受信側でPULLソケットを使用するとします。これは単純に動作するように見えますがPUSHはメッセージを全ての受信者に配布する事を思い出して下さい。受信者が2つ居て、片方が起動していない場合、半分の通知が失われてしまうことになります。PAIRは排他的であり2つ以上の接続を許可しませんのでこの様な事が起こりません。
...
スレッドとノードで異なる2つ目の重要な点は、通常スレッドの数は固定であるのに対してノードの数は可変である事です。
...
巨大なメモリブロックを頻繁に送信する様な特定のケースでは、ゼロコピーを利用することを検討してみると良いでしょう。
小さいメッセージや送受信が低頻度であればゼロコピーはコードが複雑になるだけで有意な効果を得られないかもしれません。
...
最適な回答はエンベロープを使うことです。フレーム境界を超えて一致することはありません。
