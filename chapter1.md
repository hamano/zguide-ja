...
コードは強く物静かであるだけでは十分ではありません。
コードはコードと会話し、社交的なおしゃべりでなければなりません。
...
![Publish-Subscribe](images/fig4.eps)
 ...
 * サブスクライバーは一つ以上のパブリッシャーに接続することが出来ます。一つのパブリッシャーが大量のメッセージを流して専有してしまわないように、到着したデータは制御されています。これを「均等なキューイング」と呼びます。
 ...
 * シンクのPULLソケットはワーカーからの処理結果を均等に収集します。これは*均等なキューイング*と呼びます。

![均等なキューイング](images/fig6.eps)
